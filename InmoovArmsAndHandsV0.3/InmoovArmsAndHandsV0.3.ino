...
int millisHandLeft = millis();
int goalMillisHandLeft = millisHandLeft;
int millisHandRight = millis();
int goalMillisHandRight = millisHandRight;
...
void loop() {
  ...
  goalMillisHandLeft = millis()- millisHandLeft;
  goalMillisHandRight = millis() - millisHandRight;
}

// Rotate, Clench, wave backa and forth, unclench, rotate back to origin?
void handLeftMovement() {
  switch(goalMillisHandLeft) {
    case 0:
      handLeft[0].write(posHandLeft[0][1]);
      Serial.print("Left Hand Millis: ");
      Serial.println(goalMillisHandLeft);
    break;
    case 1000:
      for(int i=1; i<=5; i++) {
        handLeft[i].write(posHandLeft[i][1]);
      }
      Serial.print("Left Hand Millis: ");
      Serial.println(goalMillisHandLeft);
    break;
    case 3000:
      handLeft[0].write(posHandLeft[0][0]);
      Serial.print("Left Hand Millis: ");
      Serial.println(goalMillisHandLeft);
    break;
    case 3250:
    ...
    case 4000:
      for(int i=1; i<=5; i++) {
        handLeft[i].write(posHandLeft[i][0]);
      }
      Serial.print("Left Hand Millis: ");
      Serial.println(goalMillisHandLeft);
    break;
...
void handRightMovement() {
  switch(goalMillisHandRight) {
    case 0:
      for (int i=1; i<=5; i++) {
        handRight[i].write(posHandRight[i][1]);
      }
      Serial.print("Right Hand Millis: ");
      Serial.println(goalMillisHandRight);
    break;
    case 2000:
      handRight[0].write(posHandRight[0][1]/2);
      Serial.print("Right Hand Millis: ");
      Serial.println(goalMillisHandRight);
    break;
    case 4000:
      handRight[1].write(posHandRight[1][0]);
      Serial.print("Right Hand Millis: ");
      Serial.println(goalMillisHandRight);
    break;
    case 6000:
      for (int i=2; i<=5; i++) {
        handRight[i].write(posHandRight[i][0]);
      }
      Serial.print("Right Hand Millis: ");
      Serial.println(goalMillisHandRight);
    break;
    case 8000:
      handRight[0].write(posHandRight[0][0]);
      Serial.print("Right Hand Millis: ");
      Serial.println(goalMillisHandRight);
    break;
    case 10000:
...
// Keeping it here just in case, but it uses delay() so avoid if possible
